...
static bool
middle_ladder_walk_chao(struct board *b, struct board *bset, group_t laddered, coord_t nextmove, enum stone lcolor)
{
	assert(board_group_info(b, laddered).libs == 1);
	...
	for (int i = 0; !is_ladder && i < libs; i++) {
		...
		/* If we just played self-atari, abandon ship. */
		/* XXX: If we were very lucky, capturing this stone will
		 * not help us escape. That should be pretty rare. */
		if (DEBUGL(6))
			fprintf(stderr, "(%d=%d) ladder atari %s (%d libs)\n", i, res, coord2sstr(ataristone, b2), board_group_info(b2, group_at(b2, ataristone)).libs);

		/////////////////////////////////////////////////////////////////////////////////
		struct move_queue qq = { .moves = 0 };
		if (res >= 0 && board_group_info(b2, group_at(b2, ataristone)).libs > 1){
			if (can_countercapture(b2, lcolor, laddered, lcolor, &qq, 0)) {
				/* We could escape by countercapturing a group.
		 		* Investigate. */
				assert(qq.moves > 0);
				for (unsigned int i = 0; i < qq.moves; i++) {
					struct board b3;
					board_copy(&b3, b2);
					bool is_ladder = middle_ladder_walk(&b3, bset, laddered, qq.move[i], lcolor);
					board_done_noalloc(&b3);
					if (!is_ladder) {
						free(bset);
						return false;
					}
...
bool
is_middle_ladder_chao(struct board *b, coord_t coord, group_t laddered, enum stone lcolor)
{
	...
	struct board b2;
	board_copy(&b2, b);
	bool is_ladder = middle_ladder_walk_chao(&b2, bset, laddered, board_group_info(&b2, laddered).lib[0], lcolor);
	board_done_noalloc(&b2);
	free(bset);
	return is_ladder;
}
